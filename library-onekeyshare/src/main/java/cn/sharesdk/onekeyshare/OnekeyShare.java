...
//				}
	...
	public void onComplete(Platform platform, int action,
		...
		if(mInsertListener!=null){
			mInsertListener.onComplete(platform,action,res);
		}
	}
	...
	public void onError(Platform platform, int action, Throwable t) {
		...
		if(mInsertListener!=null){
			mInsertListener.onError(platform,action,t);
		}
	}
	...
	public void onCancel(Platform platform, int action) {
		...
		if(mInsertListener!=null){
			mInsertListener.onCancel(platform,action);
		}
	}
					...
					case 3: {
						...
						if (resId > 0) {
							...
							if(platform.getName().equals("SinaWeibo")){
								//显示成功
								//TODO 新浪微博分享成功或取消都会回调取消 待解决
//								resId = getStringRes(context,"ssdk_oks_share_completed);
//								showNotification("action:"+msg.arg2);
							}
							showNotification(context.getString(resId));
						}
					}
...
/*******添加插入监听 用于在原有监听处理基础上插入自己的监听处理********/
	private ShareInsertListener mInsertListener;

	public void setShareInsertListener(ShareInsertListener insertListener){
		this.mInsertListener = insertListener;
	}
	public interface ShareInsertListener{
		void onComplete(Platform platform, int action,
						HashMap<String, Object> res);
